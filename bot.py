...
class Bot:
    ...
    def _get_tweet(self):
        ...
        while tweet == '':
            ...
            temp = self._generate_text(num_words).split(self._random_punc())
            ...
            for i in range(len(temp), -1, -1):
                pos_tweet = self._random_punc().join(temp[k:(i + 1)])
                if len(pos_tweet) < 280 and len(temp) > 20:
                    tweet = pos_tweet + self._random_punc()
                    break
